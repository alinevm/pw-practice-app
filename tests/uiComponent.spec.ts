...
test.describe("Form Layouts page", () => {
  ...
  test("radio buttons", async ({ page }) => {
    ...
    await usingTheGridForm
      ...
      .check({ force: true }); //force: true will bypass in case the ratio button element is hidden
...
test("chekboxes", async ({ page }) => {
  await page.getByText("Modal & Overlays").click();
  await page.getByText("Toastr").click();

  await page
    .getByRole("checkbox", { name: "Hide on click" })
    .uncheck({ force: true }); //if already unchecked, it will not do anything
  await page
...
test("lists and dropdowns", async ({ page }) => {
  ...
  //   page.getByRole('listitem') //when teh list has LI tag
...
test("browser dialog box", async ({ page }) => {
  ...
  await page
    ...
    .locator(".nb-trash")
    .click();
  await expect(page.locator("tbody tr").first()).not.toHaveText("gmail");
});
...
test("web tables", async ({ page }) => {
  ...
  //get the row by the value of specific column
...
test("sliders", async ({ page }) => {
  ...
  await page.mouse.down(); //click on the left button on the mouse, engage the mouse
